...
from drf_contenttype_contracts.policies.base import (
...
def normalize_optional_field_names(fields: Optional[Iterable[str]]) -> Optional[FrozenSet[str]]:
    if fields is None:
        return None
...
def resolve_model_reference(model_ref):
    ...
    try:
        return django_apps.get_model(app_label, model_name, require_ready=False)
    except (ImproperlyConfigured, LookupError, RuntimeError, ValueError):
        return None
...
class ContentTypeContract:
    ...
    filter_fields: Optional[FrozenSet[str]] = None
    order_fields: Optional[FrozenSet[str]] = None
...
class DeclarativeContentTypeContract:
    ...
    def as_contract(cls) -> ContentTypeContract:
        ...
        if resolved_model is not None:
            app_label = app_label or resolved_model._meta.app_label
            model_name = model_name or resolved_model._meta.model_name
        ...
        return ContentTypeContract(
            ...
            lifecycle=lifecycle,
            filter_fields=normalize_optional_field_names(cls.filter_fields),
            order_fields=normalize_optional_field_names(cls.order_fields),
        )
